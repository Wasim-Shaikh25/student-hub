  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <span className="text-xs font-semibold text-primary">{c.resolutionConfidence}% resolved</span>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
        <span className="rounded-md bg-muted px-2 py-1">{c.institution}</span>
        <span className="rounded-md bg-muted px-2 py-1">{c.category}</span>
      </div>
      <div className="mt-4 flex items-center gap-4 text-sm text-muted-foreground">
        <span className="flex items-center gap-1">
          <Users className="size-4" />
          {c.affectedCount.toLocaleString()} affected
        </span>
        <span className="flex items-center gap-1">
