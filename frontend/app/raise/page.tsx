...
import { getCategories, getInstitutions, getLocations } from '@/lib/queries'
...
export default async function RaisePage() {
  ...
  const [categories, institutions, locations] = await Promise.all([
    getCategories(),
    getInstitutions(),
    getLocations(),
  ])
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="mt-6 rounded-2xl border bg-card p-6 shadow-sm">
        <CaseForm categories={categories} institutions={institutions} locations={locations} />
      </div>
    </div>
  )
}
